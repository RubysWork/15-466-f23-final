...
if (maek.OS === "windows") {
  ...
  maek.options.CPPFlags.push(
    `-O2`, //optimize
    //include paths for nest libraries:
    `-I${NEST_LIBS}/SDL2/include/SDL2`,
    `-D_THREAD_SAFE`, //the output of sdl-config --cflags
    `-I${NEST_LIBS}/glm/include`,
    `-I${NEST_LIBS}/libpng/include`,
    `-I${NEST_LIBS}/opusfile/include`,
    `-I${NEST_LIBS}/libopus/include`,
  ...
  maek.options.LINKLibs.push(
    ...
    `-lpthread`,
    `-lrt`,
    `-lGL`, //the output of sdl-config --static-libs
    `-L${NEST_LIBS}/libpng/lib`,
    `-lpng`,
    `-L${NEST_LIBS}/zlib/lib`,
    `-lz`,
    `-L${NEST_LIBS}/opusfile/lib`,
    `-lopusfile`,
    `-L${NEST_LIBS}/libopus/lib`,
    `-lopus`,
    `-L${NEST_LIBS}/libogg/lib`,
    `-logg`,
    `-L${NEST_LIBS}/harfbuzz/lib`,
    `-lharfbuzz`,
    `-L${NEST_LIBS}/freetype/lib`,
    `-lfreetype`
  );
  ...
  maek.options.LINKLibs.push(
    ...
    `IOKit`,
    `-framework`,
    `OpenGL`, //the output of sdl-config --static-libs
    `-L${NEST_LIBS}/libpng/lib`,
    `-lpng`,
    `-L${NEST_LIBS}/zlib/lib`,
    `-lz`,
...
let copies = [
  ...
  maek.COPY(
    `${NEST_LIBS}/libpng/dist/README-libpng.txt`,
    `dist/README-libpng.txt`
  ),
  maek.COPY(`${NEST_LIBS}/glm/dist/README-glm.txt`, `dist/README-glm.txt`),
  maek.COPY(
    `${NEST_LIBS}/libopus/dist/README-libopus.txt`,
    `dist/README-libopus.txt`
  ),
  maek.COPY(
    `${NEST_LIBS}/opusfile/dist/README-opusfile.txt`,
    `dist/README-opusfile.txt`
  ),
  maek.COPY(
    `${NEST_LIBS}/libogg/dist/README-libogg.txt`,
    `dist/README-libogg.txt`
  ),
  maek.COPY(
    `${NEST_LIBS}/harfbuzz/dist/README-harfbuzz.txt`,
    `dist/README-harfbuzz.txt`
  ),
  maek.COPY(
    `${NEST_LIBS}/freetype/dist/README-freetype.txt`,
    `dist/README-freetype.txt`
  ),
];
if (maek.OS === "windows") {
...
const show_meshes_exe = maek.LINK(
  [...show_meshes_names, ...common_names],
  "scenes/show-meshes"
);
const show_scene_exe = maek.LINK(
  [...show_scene_names, ...common_names],
  "scenes/show-scene"
);
...
function init_maek() {
  ...
  function combineOptions(localOptions) {
    ...
    for (const key of Object.keys(maek.options)) {
      if (!(key in combined))
        throw new Error(`ERROR: '${key}' (in maek.options) not recognized.`);
    ...
    for (const key of Object.keys(localOptions)) {
      if (!(key in combined))
        throw new Error(`ERROR: '${key}' (in local options) not recognized.`);
  ...
  maek.COPY = (srcFile, dstFile) => {
    if (typeof srcFile !== "string")
      throw new Error("COPY: from should be a single file.");
    if (typeof dstFile !== "string")
      throw new Error("COPY: to should be a single file.");
    const task = async () => {
      try {
        ...
        throw new BuildError(
          `Failed to copy '${srcFile}' to '${dstFile}': ${e}`
        );
      }
    };
  ...
  maek.CPP = (cppFile, objFileBase, localOptions = {}) => {
    ...
    if (typeof objFileBase === "undefined") {
      objFileBase = path.relative(
        "",
        options.objPrefix + cppFile.replace(/\.[^.]*$/, "")
      );
    }
    ...
    async function loadDeps() {
      ...
      if (maek.OS === "windows") {
        ...
        paths = paths.map((path) =>
          winpath.relative("", path).split("\\").join("/")
        );
        paths = paths.sort();
        return paths;
      } else {
    ...
    const task = async () => {
      ...
      await run(command, `${task.label}: compile + prerequisites`, async () => {
        return {
          read: [...(await loadDeps())],
    ...
    if (objFile in maek.tasks) {
      throw new Error(
        `Task ${task.label} purports to create ${objFile}, but ${maek.tasks[objFile].label} already creates that file.`
      );
    }
  ...
  maek.LINK = (objFiles, exeFileBase, localOptions = {}) => {
    ...
    const task = async () => {
      ...
      await run(linkCommand, `${task.label}: link`, async () => {
        return {
          read: [...objFiles],
          written: [exeFile],
        };
    ...
    if (exeFile in maek.tasks) {
      throw new Error(
        `Task ${task.label} purports to create ${exeFile}, but ${maek.tasks[exeFile].label} already creates that file.`
      );
    }
  ...
  function loadCache() {
    try {
      const loaded = JSON.parse(
        fs.readFileSync(maek.CACHE_FILE, { encoding: "utf8" })
      );
      let assigned = 0;
      let removed = 0;
      ...
      if (maek.VERBOSE)
        console.log(
          ` -- Loaded cache from '${maek.CACHE_FILE}'; had ${assigned} valid entries and ${removed} invalid ones.`
        );
    } catch (e) {
  ...
  function saveCache() {
    if (maek.VERBOSE)
      console.log(
        ` -- Writing cache with ${Object.keys(cache).length} entries to '${
          maek.CACHE_FILE
        }'...`
      );
    fs.writeFileSync(maek.CACHE_FILE, JSON.stringify(cache), {
      encoding: "utf8",
    });
  }
      ...
      const hash = await new Promise((resolve, reject) => {
        ...
        fs.readFile(file, (err, data) => {
          ...
          if (err) {
            ...
            if (err.code != "ENOENT") {
              console.warn(
                `Failed to hash ${file} because of unexpected error ${err}`
              ); //DEBUG
            }
            resolve(`x`);
          } else {
  ...
  maek.update = async (targets) => {
    const before = performance.now();
    console.log(
      ` -- Maek v0.2 on ${maek.OS} with ${
        maek.JOBS
      } max jobs updating '${targets.join("', '")}'...`
    );
    ...
    function need(target, from) {
      if (!(target in tasks)) {
        ...
        if (target[0] === ":") {
          ...
          throw new BuildError(
            `Target '${target}' (requested by ${from}) is abstract but doesn't have a task.`
          );
        }
        ...
        const task = async () => {
          try {
            ...
            throw new BuildError(
              `Target '${target}' (requested by ${from}) doesn't exist and doesn't have a task to make it.`
            );
          }
        };
        task.depends = [];
    ...
    async function launch(task) {
      ...
      for (const depend of task.depends) {
        if (tasks[depend].failed) {
          ...
          console.assert(
            tasks[depend].finished,
            "all depends should be failed or finished"
          );
        }
      }
      if (failedDepends.length) {
        ...
        if (maek.VERBOSE)
          console.error(
            `!!! SKIPPED [${task.label}] because target(s) ${failedDepends.join(
              ", "
            )} failed.`
          );
      }
      try {
        ...
        if (e instanceof BuildError) {
          console.error(
            `\x1b[91m!!! FAILED [${task.label}] ${e.message}\x1b[0m`
          );
          task.failed = true;
    ...
    if (!failed) {
      console.log(
        ` -- SUCCESS: Target(s) '${targets.join(
          "', '"
        )}' brought up to date in ${((after - before) / 1000.0).toFixed(
          3
        )} seconds.`
      );
    } else {
      if (skipped.length) {
        if (CANCEL_ALL_TASKS) {
          console.log(
            `!!! SKIPPED ${skipped.length} tasks because of failure above.`
          );
        } else {
          console.log(
            `\x1b[91m!!! FAILED: tasks ${skipped.join(
              ", "
            )} were never run (circular dependancy).\x1b[0m`
          );
        }
      } else {
    ...
    if (maek.VERBOSE) {
      ...
      console.log(
        `\x1b[35m  . hashCache ended up with ${
          Object.keys(hashCache).length
        } items and handled ${hashCacheHits} hits.\x1b[0m`
      );
      console.log(
        `\x1b[35m  . hashFiles spent ${t(hashLoadTime)} seconds loading and ${t(
          hashComputeTime
        )} hashing.\x1b[0m`
      );
      console.log(
        `\x1b[35m  . run spent ${t(runTime)} seconds running commands.\x1b[0m`
      );
    }
  ...
  process.nextTick(() => {
    ...
    if (maek.TARGETS.length === 0) {
      console.warn(
        "No targets specified on command line and no default targets."
      );
    }
